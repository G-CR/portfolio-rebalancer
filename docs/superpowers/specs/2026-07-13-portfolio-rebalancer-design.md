...
应用采用桌面优先、安静且信息密集的工具型界面。左侧为固定导航，顶部显示最近数据更新时间和异常状态。页面不包含营销内容。

### 15.1 总览

- 核心池当前市值、总成本、浮动盈亏和浮动收益率。
- 价格影响和汇率影响汇总。
- 五类资产的目标比例、实际占比、剔汇率占比和偏离。
- 最近行情时间、数据来源和过期警告。
- 最近一次再平衡和基准建立时间。

### 15.2 资产配置

- 编辑资产类别、目标比例、启用状态、顺序和备注。
- 保存前实时校验目标比例之和是否为 100%。
- 禁止删除仍有关联持仓或历史快照的类别，只允许停用。

### 15.3 持仓与成本

- 表格展示标的、账户、币种、份额、成本价、成本汇率、当前价和当前汇率。
- 提供初始录入、追加买入、部分卖出、人工修正和查看调整记录。
- 当前价格和汇率可手动覆盖，并清楚标注数据来源。

### 15.4 盈亏分析

- 按资产类别和标的展示成本、市值、浮动盈亏、收益率、价格影响和汇率影响。
- 支持人民币金额和交易币种金额查看。
- 盈亏颜色同时搭配正负号和文字，避免只依赖颜色表达。

### 15.5 再平衡

- 输入临时可用人民币和美元，以及本次策略参数。
- 显示含汇率建议和剔汇率模拟对照。
- 展示交易前后占比、交易建议、换汇需求、剩余现金和警告。
- 支持保存方案、标记取消和完成再平衡。
...
- 配置数据源优先级和刷新时间。
- 展示可选密钥是否已配置和验证结果，不回显完整密钥。
- 手动刷新、查看失败原因、设置或取消手动覆盖。
