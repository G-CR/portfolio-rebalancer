    ...
    <div className={styles.tableWrap}>
      <table className={styles.table}>
        ...
        <tbody>{items.map((item) => <tr key={item.key}>
          <th scope="row"><span className={styles.dataIdentity}><strong>{item.symbol}</strong><span>{item.data_type === "fx" ? "汇率" : "行情"} · {item.currency}</span></span></th>
          ...
          <td><span className={styles.statusStack}><strong className={item.status === "stale" || item.status === "failed" ? styles.badStatus : item.status === "manual" ? styles.manualStatus : styles.goodStatus}>{statusLabel(item.status)}</strong>{item.error_summary ? <small>{item.error_summary}</small> : item.note ? <small>{item.note}</small> : null}</span></td>
