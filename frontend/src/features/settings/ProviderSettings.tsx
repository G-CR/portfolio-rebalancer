...
function ProviderRow({ item }: { item: ProviderSetting }) {
  ...
  return (
    <fieldset className={styles.providerRow} aria-label={`${item.display_name} 设置`}>
      ...
      <FormField label="优先级"><select value={priority} onChange={(event) => setPriority(event.target.value)}>{[1, 2, 3, 4].map((value) => <option value={value} key={value}>{value}</option>)}</select></FormField>
