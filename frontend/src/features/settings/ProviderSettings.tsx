...
function ProviderRow({ item }: { item: ProviderSetting }) {
  ...
  return (
    <fieldset className={styles.providerRow} aria-label={`${item.display_name} 设置`}>
      ...
      <FormField label="优先级"><select value={priority} onChange={(event) => setPriority(event.target.value)}>{[1, 2, 3, 4, 5].map((value) => <option value={value} key={value}>{value}</option>)}</select></FormField>
      {item.requires_key ? <FormField label="API 密钥"><input type="password" autoComplete="new-password" value={apiKey} placeholder={item.masked_key ?? "输入新密钥"} onChange={(event) => setApiKey(event.target.value)} /></FormField> : <div className={styles.noKey}><ShieldCheck size={16} aria-hidden="true" />公开数据源</div>}
      {item.requires_key ? <label className={styles.enabled}><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} />启用</label> : <span className={styles.enabled}><CheckCircle2 size={15} aria-hidden="true" />启用</span>}
      <div className={styles.providerActions}><button type="button" className={styles.secondary} onClick={() => void submit()} disabled={save.isPending}><Save size={15} aria-hidden="true" />保存 {item.display_name}</button>{item.requires_key && item.key_status === "configured" ? <button type="button" className={styles.ghost} onClick={() => void test.mutateAsync(item.provider)} disabled={test.isPending}><KeyRound size={15} aria-hidden="true" />测试</button> : null}</div>
