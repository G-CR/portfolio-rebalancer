...
describe("sale, correction, and history drawers", () => {
  ...
  it("clears a stale sale preview and invalidates affected caches", async () => {
    ...
    const { queryClient } = renderWithProviders(
      <SaleDrawer holding={holdingFixture} open onClose={() => undefined} />,
      { handlers: [
        http.post(`/api/cost-adjustments/${holdingFixture.id}/preview-sell`, () => HttpResponse.json({
          holding_id: holdingFixture.id,
          holding_version: 1,
    ...
    queryClient.setQueryData(costAdjustmentsQueryKey(holdingFixture.id), { items: [] });

    await user.type(screen.getByRole("textbox", { name: "卖出份额" }), "2");
    await user.click(screen.getByRole("button", { name: "预览卖出调整" }));
    await user.click(await screen.findByRole("button", { name: "确认卖出调整" }));

    expect(await screen.findByRole("alert")).toHaveTextContent("持仓已发生变化，请重新生成卖出调整预览");
    expect(screen.getByRole("button", { name: "确认卖出调整" })).toBeDisabled();
  ...
  it("clears a stale correction preview and requires regeneration", async () => {
    const user = userEvent.setup();
    renderWithProviders(
      <CorrectionDrawer holding={holdingFixture} open onClose={() => undefined} />,
      { handlers: [
        http.post(`/api/cost-adjustments/${holdingFixture.id}/preview-correction`, () => HttpResponse.json({
          holding_id: holdingFixture.id,
          holding_version: 1,
          operation: "manual_correction",
          before: basis,
          after: { ...basis, quantity: "11.0000", total_cost_cny: "40299.55" },
          fee: null,
          note: "券商对账",
          adjustment_id: null,
        })),
    ...
    const quantity = screen.getByRole("textbox", { name: "修正后份额" });
    await user.clear(quantity);
    await user.type(quantity, "11.0000");
    await user.type(screen.getByRole("textbox", { name: "修正原因" }), "券商对账");
    await user.click(screen.getByRole("button", { name: "预览人工修正" }));
    await user.click(await screen.findByRole("button", { name: "确认人工修正" }));

    expect(await screen.findByRole("alert")).toHaveTextContent("持仓已发生变化，请重新生成人工修正预览");
    expect(screen.getByRole("button", { name: "确认人工修正" })).toBeDisabled();
    expect(screen.queryByText("11.0000")).not.toBeInTheDocument();
  });
  ...
  it("clears a stale restore preview, refetches history, and offers regeneration", async () => {
    const user = userEvent.setup();
    ...
    renderWithProviders(
      <AdjustmentHistoryDrawer holding={holdingFixture} open onClose={() => undefined} />,
      { handlers: [
        http.get(`/api/cost-adjustments/${holdingFixture.id}`, () => {
          historyRequests += 1;
    ...
    await user.click(within(item).getByRole("button", { name: "恢复到此状态" }));
    const restoreDialog = await screen.findByRole("dialog", { name: "恢复 SPY 成本状态" });
    await user.click(within(restoreDialog).getByRole("button", { name: "确认恢复为新修正" }));

    expect(await within(restoreDialog).findByRole("alert")).toHaveTextContent("持仓已发生变化，请重新生成恢复预览");
    expect(within(restoreDialog).getByRole("button", { name: "确认恢复为新修正" })).toBeDisabled();
    expect(within(restoreDialog).getByRole("button", { name: "重新生成恢复预览" })).toBeEnabled();
